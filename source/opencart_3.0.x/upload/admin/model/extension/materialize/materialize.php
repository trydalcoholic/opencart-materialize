...
class ModelExtensionMaterializeMaterialize extends Model {
	public function install() {
		$check_materialize_colors		= $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "materialize_colors';");
		$check_materialize_colors_text	= $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "materialize_colors_text';");
		$check_social_networks			= $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "materialize_social_networks';");
		...
		if (count($check_materialize_colors->rows) <= 0) {
			$this->db->query("
		...
		if (count($check_materialize_colors_text->rows) <= 0) {
			$this->db->query("
		...
		if (count($check_social_networks->rows) <= 0) {
			$this->db->query("
	...
	public function checkUpdate() {
		...
		if (count($check_materialize_colors->rows) && count($check_materialize_colors_text->rows) && count($check_social_networks->rows) && count($check_information_top->rows) <= 0) {
			$check_updates = false;
