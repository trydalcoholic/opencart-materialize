...
class ModelExtensionMTMaterializeThemeMTMaterialize extends Model {
	public function install() {
		$this->db->query("
			CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "mt_materialize_colors` (
		...
		$this->db->query("
			INSERT INTO `" . DB_PREFIX . "mt_materialize_colors` (`name`, `hex`) VALUES
	...
	public function uninstall() {
		$this->db->query("
			DROP TABLE IF EXISTS
				`" . DB_PREFIX . "mt_materialize_colors`
			;
		");
	}

	public function getMaterialColors() {
		$colors = $this->cache->get('admin.mt_template.colors');

		if (!$colors) {
			$colors = array();

			$query = $this->db->query("SELECT `name`, `hex` FROM `" . DB_PREFIX . "mt_materialize_colors` LIMIT 257");

			foreach ($query->rows as $result) {
				$colors[$result['name']] = $result['hex'];
			}

			$this->cache->set('admin.mt_template.colors', $colors);
		}
