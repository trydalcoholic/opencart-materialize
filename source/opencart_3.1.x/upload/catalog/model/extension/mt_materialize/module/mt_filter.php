...
class ModelExtensionMTMaterializeModuleMTFilter extends Model {
	...
	public function getOptions($data) {
		...
		$sql = "SELECT DISTINCT o.option_id, od.name FROM " . DB_PREFIX . "option o LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) LEFT JOIN " . DB_PREFIX . "product_option po ON (o.option_id = po.option_id)";

		if (!empty($data['category_id'])) {
			$sql .= " LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (po.product_id = p2c.product_id)";
		}

		$sql .= " WHERE od.language_id = '" . (int)$this->config->get('config_language_id') . "' AND (o.type = 'radio' OR o.type = 'checkbox' OR o.type = 'select')";
		...
		foreach ($query->rows as $result) {
			...
			foreach ($option_value_data_query->rows as $option_value) {
				$option_value_data[(int)$option_value['option_value_id']] = [
			...
			$option_data[(int)$result['option_id']] = [
