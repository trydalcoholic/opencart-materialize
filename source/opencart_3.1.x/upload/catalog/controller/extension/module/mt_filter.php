...
class ControllerExtensionModuleMTFilter extends Controller {
	private static $option_image_size = [
		'width_25'	=> 25,
		'width_40'	=> 40,
		'height_25'	=> 25,
		'height_40'	=> 40
	...
	public function index() {
		...
		$placeholder25 = $this->model_tool_image->resize('placeholder.png', 25, 25);
		$placeholder40 = $this->model_tool_image->resize('placeholder.png', 40, 40);
		...
		if (!empty($filters['sub_categories']['status'])) {
			...
			$sub_categories_data = $this->model_extension_mt_materialize_module_mt_filter->getSubCategories($filter_settings);
			...
			if ($sub_categories_data) {
				...
				foreach ($sub_categories_data as $sub_category) {
					if (!empty($filters['sub_categories']['image']) && is_file(DIR_IMAGE . html_entity_decode($sub_category['image'], ENT_QUOTES, 'UTF-8'))) {
						$image = $this->model_tool_image->resize(html_entity_decode($sub_category['image'], ENT_QUOTES, 'UTF-8'), $imageSelect ? self::$option_image_size['width_40'] : self::$option_image_size['width_25'], $imageSelect ? $image_height = self::$option_image_size['height_40'] : self::$option_image_size['height_25']);
					} else {
						$image = $imageSelect ? $placeholder40 : $placeholder25;
					}

					if ($config_product_count) {
						$filter_data = [
							'category_id'			=> (int)$product_category,
							'sub_category_filter'	=> (array)$sub_category['category_id'],
						];
					...
					$input_values[(int)$sub_category['category_id']] = [
						'mt_filter_id'				=> 'mt-filter-sub-category-' . (int)$sub_category['category_id'],
						'name'						=> $sub_category['name'],
						'value'						=> (int)$sub_category['category_id'],
						'image'						=> $image,
						'preselected'				=> !empty($preselected) ? in_array((int)$sub_category['category_id'], $preselected) : false,
			...
			if ($manufacturers_data) {
				...
				foreach ($manufacturers_data as $manufacturer) {
					if (!empty($filters['manufacturers']['image']) && is_file(DIR_IMAGE . html_entity_decode($manufacturer['image'], ENT_QUOTES, 'UTF-8'))) {
						$image = $this->model_tool_image->resize(html_entity_decode($manufacturer['image'], ENT_QUOTES, 'UTF-8'), $imageSelect ? self::$option_image_size['width_40'] : self::$option_image_size['width_25'], $imageSelect ? $image_height = self::$option_image_size['height_40'] : self::$option_image_size['height_25']);
					} else {
						$image = $imageSelect ? $placeholder40 : $placeholder25;
					}
					...
					$input_values[(int)$manufacturer['manufacturer_id']] = [
			...
			if ($options_data) {
				...
				foreach ($options_data as $option_data) {
					foreach ($option_data['option_value_data'] as $option_value) {
						if (!empty($filters['options']['image']) && is_file(DIR_IMAGE . html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8'))) {
							$image = $this->model_tool_image->resize(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8'), $imageSelect ? self::$option_image_size['width_40'] : self::$option_image_size['width_25'], $imageSelect ? $image_height = self::$option_image_size['height_40'] : self::$option_image_size['height_25']);
						} else {
							$image = $imageSelect ? $placeholder40 : $placeholder25;
						}
						$data['dqwdqwdqw'] = $option_data;

						$option_filter[(int)$option_data['option_id']] = (int)$option_value['option_value_id'];
						...
						$input_values[$option_data['option_id']] = [
							...
							'name'						=> $option_value['value_name'],
							'value'						=> $option_value['option_value_id'],
							'image'						=> !empty($filters['options']['image']) ? $image : false,
							'preselected'				=> !empty($preselected[(int)$option_data['option_id']]) ? in_array((int)$option_value['option_value_id'], $preselected[(int)$option_data['option_id']]) : false,
							'product_count'				=> (int)$product_count,
							'product_count_disabled'	=> isset($product_count) ? false : true
						];
					}

					$options[$option_data['option_name']] = [
						/*'option_id'	=> (int)$option_data['option_id'],*/
						'input_name'	=> $option_data['option_name'],
						'input_values'	=> $input_values
					];
				}
			}
