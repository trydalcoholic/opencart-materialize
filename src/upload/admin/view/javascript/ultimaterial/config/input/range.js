export default class URange {
  ...
  _handleRange(event) {
    ...
    target.style.setProperty('--slider-percent', 'calc(100% * ' + value + ' / ' + target.getAttribute('max') + ')');
    output.innerText = target.dataset.propertyValue.replace(/\$\{value\}/i, target.value);
